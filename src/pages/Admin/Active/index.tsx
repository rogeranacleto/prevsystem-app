...
const {activeMaintenance} = useContext(CardContext);
...
useEffect(() => {
    if(maintenanceRecord.length > 0){
        activeMaintenance(maintenanceRecord.length)
    }
},[maintenanceRecord])
...
<div className="bg-black max-w-screen min-h-screen rounded-2xl p-7">
            ...
            <div>
                <h1 className="text-white font-medium text-lg">Seção de Manutenções <span className="text-[#26a01b] font-bold">Ativas</span></h1>
            </div>
                ...
                <div className="p-5">
                    ...
                    {filteredMaintenanceRecord.map((item) => (
                        <tr className="bg-black hover:scale-101 duration-300 ease-in-out" key={item.id}>
                            ...
                            <span className="bg-[#26a01b] font-bold px-4 py-2 rounded-2xl">{item.status}</span>
                            </button>
                            </td>
                    ...
                    {maintenanceRecord.length === 0 && (<tr><td colSpan={6} className="p-5 text-center text-gray-400/50">Nenhuma manutenção ativa no momento.</td></tr>)}
                    </table>
                </div>
            </div>
        </div>
